...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    // Symmetric upper buffer — west-of-UTC trips have a final local day
    // that extends to ~05:00 UTC the "next" iCal day; trip-ID matching
    // is the source of truth, so the fallback can be lenient without
    // re-introducing adjacent-trip bleed (the IDs would mismatch and
    // skip the buffered overlap path entirely).
...
function buildLodgingPrimarySegments(trip, tripFlights, tripStays, segments) {
  ...
  // flight segments after dedup-clip.
  const preLodgingEnd = (() => {
    let latest = null;
    for (const seg of segments) {
      if (!seg.label?.startsWith(`${trip.summary} - `)) continue;
