...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    // Strict UPPER bound for tripFlights — the adjacent-trip bleed bug was
    // about flights at tripEnd+buffer (next trip's outbound flight), so
    // `< tripEnd` is correct. Keep a buffer on the LOWER bound: trip start
    // is iCal midnight UTC, but a trip in an east-of-UTC timezone (e.g.
    // Tokyo, Berlin) has a real first calendar day that begins hours
    // before tripStart UTC. Without the lower buffer, a legitimate
    // first-day arrival on the user's local clock can be excluded.
    const FLIGHT_LOWER_BUFFER_MS = 24 * 60 * 60 * 1000;
    const tripFlights = flights.filter(f => {
      const t = f.arrivalDate.getTime();
      return t >= tripStart.getTime() - FLIGHT_LOWER_BUFFER_MS && t < tripEnd.getTime();
    });
    ...
    const tripStays = stays.filter(s => {
      if (!s.checkinDate) {
        ...
        return checkoutMs <= tripEnd.getTime();
      }
