...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    // Asymmetric trip-window buffers for tripFlights:
    //   - LOWER: 24h. Trip startDate is iCal midnight UTC; an east-of-UTC
    //     trip (Tokyo, Berlin) has a real first local day that begins
    //     hours before tripStart UTC, so a legitimate first-day arrival
    //     on the user's local clock can be before tripStart UTC.
    //   - UPPER: 12h. Symmetric to the above for west-of-UTC trips
    //     (a Chicago user's last local day extends to ~05:00 UTC the
    //     "next" iCal day). 12h is small enough that next-trip outbound
    //     flights — which the adjacent-trip-bleed bug was about, and
    //     which usually depart 24h+ after the previous trip ends — stay
    //     excluded.
    const FLIGHT_LOWER_BUFFER_MS = 24 * 60 * 60 * 1000;
    const FLIGHT_UPPER_BUFFER_MS = 12 * 60 * 60 * 1000;
    const tripFlights = flights.filter(f => {
      const t = f.arrivalDate.getTime();
      return t >= tripStart.getTime() - FLIGHT_LOWER_BUFFER_MS
        && t < tripEnd.getTime() + FLIGHT_UPPER_BUFFER_MS;
    });

    // Timezone-less stays are kept here so they still inform the lodging
    // boundary scan (firstCheckinDate, lastStay, between-stay gaps); they
    // are skipped only when emitting lodging segments in
    // `buildLodgingSegments`. Excluding them earlier would let flights
    // override days the user was actually at a hotel we couldn't resolve.
    //
    // Unmatched-checkout stays (checkinDate === null) are kept *only* if
    // their checkout is well past the trip start. Carry-over hotels from
    // the previous trip typically check out in the first hours of the new
    // trip; legitimate within-trip orphans (TripIt missing a check-in
    // event) check out later. The 12-hour cutoff separates the two cases
    // pragmatically — carry-overs land before noon on day 1, real
    // mid-trip stays end on day 2 or later.
    const ORPHAN_CARRY_OVER_CUTOFF_MS = 12 * 60 * 60 * 1000;
    // Orphan upper buffer (symmetric to FLIGHT_UPPER_BUFFER_MS): a
    // west-of-UTC last-local-day orphan checkout can be hours past
    // tripEnd UTC. Paired stays keep the strict upper bound — the
    // adjacent-trip bleed was specifically about a next-trip hotel's
    // check-in landing in the buffer window.
    const ORPHAN_UPPER_BUFFER_MS = 12 * 60 * 60 * 1000;
    const tripStays = stays.filter(s => {
      if (!s.checkinDate) {
        if (!s.checkoutDate) return false;
        const checkoutMs = s.checkoutDate.getTime();
        if (checkoutMs <= tripStart.getTime() + ORPHAN_CARRY_OVER_CUTOFF_MS) return false;
        return checkoutMs <= tripEnd.getTime() + ORPHAN_UPPER_BUFFER_MS;
      }
      const stayStart = s.checkinDate.getTime();
      const stayEnd = (s.checkoutDate || s.checkinDate).getTime();
      return stayStart < tripEnd.getTime() && stayEnd > tripStart.getTime();
    });
    ...
    if (tripStays.length > 0) {
      ...
      produced = segments.length > before;
    ...
    // GEO fallback if neither lodging-primary nor flight-only produced
    // anything — e.g., the only stay has no resolved timezone and there
    // are no usable flights. Without this, the trip would silently get
    // no segment at all even though trip.geo can give a last-resort TZ.
    if (!produced) {
 ...
 *
 * Each non-final stay ends at the next stay's check-in (or the next stay's
 * check-out if check-in is missing). The final stay ends at its own
 * check-out, falling back to rangeEnd if it has none. All segment endpoints
 * are clamped to [rangeStart, rangeEnd] as defense-in-depth so accidental
 * out-of-window inputs can't bleed into the next trip.
 *
 * Stays without a resolved timezone are skipped here (they can't produce a
 * segment), but the caller keeps them in the array so they still influence
 * boundary calculations like `firstCheckinDate` and between-stay gaps.
 */
function buildLodgingSegments(stays, rangeStart, rangeEnd, tripSummary, segments) {
  ...
  for (let i = 0; i < stays.length; i++) {
    ...
    let nextResolvedIdx = -1;
    for (let k = i + 1; k < stays.length; k++) {
      if (stays[k].timezone) { nextResolvedIdx = k; break; }
    }

    const rawStart = stay.checkinDate || lastEmittedEnd || rangeStart;
    let rawEnd;
    if (nextResolvedIdx >= 0) {
      rawEnd = stays[nextResolvedIdx].checkinDate || stays[nextResolvedIdx].checkoutDate;
    } else if (i < stays.length - 1) {
      // No further resolved stays, but TZ-less stays follow. Extend through
      // the last TZ-less stay's checkout (or rangeEnd) so trailing
      // unresolved-hotel days don't fall back to the default timezone.
      const lastStay = stays[stays.length - 1];
      rawEnd = lastStay.checkoutDate || rangeEnd;
    } else {
