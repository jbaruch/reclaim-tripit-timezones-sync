...
function buildLodgingPrimarySegments(trip, tripFlights, tripStays, segments) {
  ...
  // outbound journey. Anchor only on a known check-in — using a
  // checkout-only stay's date as cutoff would mis-classify flights that
  // could have occurred during the stay as pre-lodging. When all stays
  // are orphan checkouts, skip this section entirely; the orphan claims
  // the trip-start → checkout window itself.
