...
export function deduplicateSegments(segments) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (prev.timezone === curr.timezone && prev.endDate >= curr.startDate) {
      ...
      // Clip previous segment if it overlaps with the current one. Clip
      // both the date-only and ISO-datetime boundaries together so the
      // shapes stay consistent — a clipped segment whose endDate moves
      // earlier but whose endDateTime still points at the original end
      // would lie about its own boundary to a datetime-aware consumer.
