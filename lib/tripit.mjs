...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    if (!produced) {
      ...
      // Bound GEO to whatever stay-window we can recover. Single-sided
      // stays (only checkin OR only checkout) still get a defensible
      // window — fall back to the same side for the missing one.
      ...
      const stayStart = checkins.length > 0 ? Math.min(...checkins)
        : checkouts.length > 0 ? Math.min(...checkouts) : null;
      const stayEnd = checkouts.length > 0 ? Math.max(...checkouts)
        : checkins.length > 0 ? Math.max(...checkins) : null;
      if (stayStart !== null && stayEnd !== null) {
        const tz = resolveFromGeo(trip.geo);
        if (tz) {
          segments.push({
            startDate: formatDate(new Date(stayStart)),
            endDate: formatDate(new Date(stayEnd)),
            timezone: tz,
            label: `${trip.summary} - ${trip.location}`,
          });
        }
