...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    const matchesTrip = (rec, recStart, recEnd) => {
      ...
      if (rec.tripId) return false;
      // Neither side has a tripId — fall back to date-window overlap with
      // a ±24h buffer for east/west-of-UTC boundary cases.
    ...
    if (!produced) {
      ...
      // Use ALL stay boundaries (checkin AND checkout) to derive the
      // window — a leading orphan stay with only a checkoutDate would
      // otherwise be missed by a checkin-only min, leaving its days
      // uncovered before the next resolved stay's check-in.
      const allBoundaries = [];
      for (const s of tripStays) {
        if (s.checkinDate) allBoundaries.push(s.checkinDate.getTime());
        if (s.checkoutDate) allBoundaries.push(s.checkoutDate.getTime());
      }
      ...
      const stayEnd = allBoundaries.length > 0 ? Math.max(...allBoundaries) : tripEndMs;
