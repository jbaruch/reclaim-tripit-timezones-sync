...
function buildLodgingSegments(stays, rangeStart, rangeEnd, tripSummary, segments) {
  ...
  for (let i = 0; i < stays.length; i++) {
    ...
    // Only consider stays that have BOTH a resolved timezone AND a real
    // check-in as the next boundary. An orphan-checkout stay (no checkin)
    // can't serve as a clean end-boundary — extending the previous segment
    // to the orphan's checkout would also collapse the orphan's own
    // segment to zero length when it iterates next.
    let nextResolvedIdx = -1;
    for (let k = i + 1; k < stays.length; k++) {
      if (stays[k].timezone && stays[k].checkinDate) { nextResolvedIdx = k; break; }
    }
    ...
    if (nextResolvedIdx >= 0) {
      ...
      // No further resolved stay-with-checkin. Extend through TRAILING
      // TZ-LESS stays only — a resolved orphan-checkout (TZ but no
      // checkin) coming after this one is going to emit its own segment,
      // so don't reach into its window or its segStart will collapse to
      // zero length when it iterates.
      let lastTzLessIdx = -1;
      for (let k = i + 1; k < stays.length; k++) {
        if (stays[k].timezone) break;
        lastTzLessIdx = k;
      }
      if (lastTzLessIdx >= 0) {
        rawEnd = stays[lastTzLessIdx].checkoutDate || rangeEnd;
      } else {
