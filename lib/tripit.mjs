...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    // Strict trip-window membership for both flights and stays. Earlier
    // versions used a ±1-day buffer to be lenient about iCal end-day
    // exclusivity, but the buffer pulled adjacent-trip flights/stays into
    // this trip's segment-builder and let them drive the timezone choice.
    // iCal trip endDate is exclusive (the day after the last trip day), so
    // `< tripEnd` is the correct inclusive-of-last-day boundary.
    const tripFlights = flights.filter(f => {
      const t = f.arrivalDate.getTime();
      return t >= tripStart.getTime() && t < tripEnd.getTime();
    });
...
function buildLodgingSegments(stays, rangeStart, rangeEnd, tripSummary, segments) {
  ...
  for (let i = 0; i < stays.length; i++) {
    ...
    const rawStart = stay.checkinDate || rangeStart;
    let rawEnd;
