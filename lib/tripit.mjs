...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    if (tripStays.length > 0) {
      ...
      // Lodging-primary only "covers" the stay window if at least one
      // stay had a resolved TZ to emit. If every stay was TZ-less,
      // gap-fill flights at the edges may exist (so produced=true) but
      // the stay days in the middle are uncovered — GEO should still
      // fill them.
      stayWindowCovered = tripStays.some(s => s.timezone);
    } else if (tripFlights.length > 0) {
      const before = segments.length;
      buildFlightOnlySegments(trip, tripFlights, segments);
      produced = segments.slice(before).some(isUseful);
      stayWindowCovered = true; // no stays to cover
    }

    // GEO fallback. Fires when:
    //   - Nothing was emitted at all (no flights, no resolvable lodging)
    //   - OR lodging-primary emitted only edge gap-fill flight segments
    //     and the stay window itself is uncovered (every stay was TZ-less)
    // Dedup later clips the GEO segment against any flight segments that
    // do exist on either side of the stay window.
    if (!produced || !stayWindowCovered) {
      ...
      } else if (!produced && tripStays.length === 0 && tripFlights.length === 0) {
