...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    //
    // Unmatched-checkout stays (checkinDate === null) are excluded — they
    // represent carry-over hotels whose check-in was in another trip; if
    // their checkout happens to fall in the morning of this trip, they
    // would otherwise switch this trip into lodging-primary mode and emit
    // a segment from `trip.startDate` in the old hotel's timezone.
    const tripStays = stays.filter(s => {
      if (!s.checkinDate) return false;
...
function buildLodgingSegments(stays, rangeStart, rangeEnd, tripSummary, segments) {
  ...
  for (let i = 0; i < stays.length; i++) {
    ...
    const rawEnd = nextResolvedIdx >= 0
      ? (stays[nextResolvedIdx].checkinDate || stays[nextResolvedIdx].checkoutDate)
