 ...
 * only fill the pre-first-checkin and post-last-checkout gaps, because TripIt
 * feeds often include companion-traveler flights to different destinations
 * that can't be distinguished structurally from the user's own flights.
 *
 * Priority:
...
/**
 * Lodging-primary build: lodging defines segments, flights fill before-first-checkin
 * and after-last-checkout gaps.
 */
function buildLodgingPrimarySegments(trip, tripFlights, tripStays, segments) {
  ...
  const firstStayDate = firstStay.checkinDate || firstStay.checkoutDate;
  const firstStayCalDate = formatDate(firstStayDate);

  // Pre-lodging gap: flights arriving before the first check-in are the
  // outbound journey. Use timestamp comparison so a same-day flight that
  // lands AFTER the first check-in is treated as between-stay instead, not
  // double-counted as both pre-lodging and between-stay.
  const preLodgingFlights = tripFlights.filter(f =>
    f.arrivalDate.getTime() <= firstStayDate.getTime()
  );
  ...
  for (let i = 0; i < sortedStays.length - 1; i++) {
    ...
    const gapStart = formatDate(checkout);
    const gapEnd = formatDate(nextCheckin);
    if (gapStart >= gapEnd) continue;

    const gapFlights = tripFlights.filter(f => {
      const arr = formatDate(f.arrivalDate);
      return arr >= gapStart && arr <= gapEnd;
    });
  ...
  if (lastStayCheckout && formatDate(lastStayCheckout) < formatDate(trip.endDate)) {
    const lastCheckoutCal = formatDate(lastStayCheckout);
    const postLodgingFlights = tripFlights.filter(f =>
      formatDate(f.arrivalDate) >= lastCheckoutCal
    );
