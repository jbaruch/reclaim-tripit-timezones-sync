...
export function extractLodging(events) {
  ...
  // Include tripId in the key so two trips at the same hotel chain on the
  // same timestamp don't collapse into one and lose the trip association.
  const dedupBy = (arr) => {
    const seen = new Set();
    return arr.filter(e => {
      const key = `${e.hotelName}|${e.date.getTime()}|${e.tripId || ''}`;
  ...
  for (const ci of uniqueCheckins) {
    // Two-pass pairing: prefer same-tripId match when this checkin has an
    // ID. Only fall back to name+date pairing when no same-ID checkout is
    // available — otherwise a one-sided missing tripId (e.g., URL-less
    // description) could let a different-trip checkout pair in by name
    // alone, recreating the cross-trip-pairing bug.
    let bestIdx = -1;
    let bestDate = null;
    const tryMatch = (allowMissingId) => {
      for (let i = 0; i < uniqueCheckouts.length; i++) {
        ...
        if (ci.tripId && !co.tripId && !allowMissingId) continue;
        if (!ci.tripId && co.tripId && !allowMissingId) continue;
...
function buildLodgingPrimarySegments(trip, tripFlights, tripStays, segments) {
  ...
  // Pre-lodging gap: flights arriving before the first lodging signal
  // are the outbound journey. Anchor on the earliest known check-in
  // when possible (using an unmatched checkout's date would mis-classify
  // flights that occurred during the lodging as pre-lodging). When the
  // only stays are orphan checkouts, fall back to the earliest checkout
  // so pre-lodging flights still get emitted; the orphan segment may
  // collapse to zero in that case but the flight TZ is preserved.
  const firstCheckinDate = sortedStays.find(s => s.checkinDate)?.checkinDate;
  const preLodgingCutoff = firstCheckinDate
    || sortedStays.find(s => s.checkoutDate)?.checkoutDate;
  if (preLodgingCutoff) {
    const preLodgingFlights = tripFlights.filter(f =>
      f.arrivalDate.getTime() <= preLodgingCutoff.getTime()
    );
  ...
  // Post-lodging gap: every flight after the latest checkout produces a
  ...
  // Use the latest known checkout across all stays — the chronologically
  // last stay might lack a checkout (open-ended check-in only) but an
  // earlier stay may have one we can anchor on.
  const allCheckouts = sortedStays
    .filter(s => s.checkoutDate)
    .map(s => s.checkoutDate.getTime());
  const lastStayCheckout = allCheckouts.length > 0
