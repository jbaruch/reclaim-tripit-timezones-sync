...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    // Stays must strictly overlap the trip window (no buffer) — a hotel
    // assigned to the next trip whose check-in lands the day after this trip
    // ends would otherwise switch this trip into lodging-primary mode and
    // anchor `firstCheckinDate` / `lastStay` on data that doesn't belong.
    //
 ...
 * are clamped to [rangeStart, rangeEnd] so the ±1-day buffer used by callers
 * to select stays can't bleed segments into the next trip.
 */
