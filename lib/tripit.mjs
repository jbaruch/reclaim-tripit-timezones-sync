...
function buildFlightOnlySegments(trip, tripFlights, segments) {
  // Clamp segments to the trip window: tripFlights includes a ±1-day buffer,
  // so a "next flight" past trip.endDate or a "first flight" before
  // trip.startDate would otherwise produce segments that bleed into adjacent
  // trips and write timezone overrides outside the intended range.
  const tripStartMs = trip.startDate.getTime();
  const tripEndMs = trip.endDate.getTime();
