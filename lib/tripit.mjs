...
export function buildTimezoneSegments(trips, flights, stays = []) {
  ...
  for (const trip of trips) {
    ...
    const matchesTrip = (rec, recStart, recEnd) => {
      ...
      // Fallback when either side lacks a tripId: any overlap with the
      // trip window. Boundary-inclusive on both sides so fixture-based
      // tests using point timestamps at tripStart/tripEnd keep working.
      return recStart <= tripEnd.getTime() && recEnd >= tripStart.getTime();
    };
