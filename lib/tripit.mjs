 ...
 *
 * In OneCLI mode (`ONECLI_URL` set) we pull the feed ourselves via global
 * `fetch` so the request routes through the undici ProxyAgent installed by
 * `lib/proxy.mjs`. node-ical's internal fetcher ignores that dispatcher, so
 * `fromURL` would bypass the credential gateway. Outside OneCLI mode we keep
 * the original `fromURL` path unchanged.
 *
 * The URL may carry a placeholder path token (`.../private/<placeholder>/…`);
 * the gateway rewrites it. Do not parse or validate the token.
 */
  ...
  if (firstCheckinDate) {
    ...
    for (let i = 0; i < preLodgingFlights.length; i++) {
      ...
      segments.push(makeSegment(
        ...
        clamp(rawEnd),
        tz,
        `${trip.summary} - ${flight.arrivalCity}`,
      ));
    }
  }

  // Pass the highest emitted boundary so far (from pre-lodging flights)
  // as the orphan rawStart fallback. Without this, an orphan with no
  // earlier resolved stay could claim back to rangeStart and eclipse
  // pre-lodging flight segments after dedup-clip. Compute it from THIS
  // trip's slice of `segments` — scanning the whole array would pick
  // up segments from earlier trips that happen to share the same
  // summary. Read `endDateTime` when present so a flight arriving at
  // 15:00Z doesn't get reduced to that calendar date's midnight; an
  // orphan stay following the flight must start at the actual arrival
  // boundary, not hours earlier.
  const preLodgingEnd = (() => {
    let latest = null;
    for (let k = preLodgingSliceStart; k < segments.length; k++) {
      const seg = segments[k];
      const endIso = seg.endDateTime ?? (seg.endDate + 'T00:00:00.000Z');
      if (!latest || endIso > latest) latest = endIso;
    }
...
function buildLodgingSegments(stays, rangeStart, rangeEnd, tripSummary, segments, initialLastEnd = null) {
  ...
  for (let i = 0; i < stays.length; i++) {
    ...
    const clampedStart = clamp(rawStart);
    const clampedEnd = clamp(rawEnd);
    segments.push(makeSegment(
      clampedStart,
      clampedEnd,
      stay.timezone,
      `${tripSummary} - ${stay.hotelName}`,
    ));
...
/**
 * Build a segment with both date-only and ISO-datetime boundaries.
 *
 * `startDate` / `endDate` (YYYY-MM-DD) preserve the original Reclaim
 * API + dedup contract. `startDateTime` / `endDateTime` (ISO 8601 UTC)
 * preserve the underlying Date's wall-clock so downstream consumers
 * (notably the nanoclaw host's `tz_state` heartbeat-advisory walker)
 * can resolve at sub-day granularity instead of flipping at UTC
 * midnight when the trip's first segment is `from: <today>`.
 *
 * Trip-level all-day VEVENTs produce midnight-UTC Date objects; the
 * datetime fields still populate truthfully (`...T00:00:00.000Z`),
 * which is the right answer for date-only origins and lets the host
 * treat every segment uniformly without sentinel checks.
 */
function makeSegment(start, end, timezone, label) {
  return {
 ...
 *
 * `today` (YYYY-MM-DD) defaults to the current date and exists so callers —
 * chiefly tests — can pin the reference date and stay deterministic as real
 * time advances.
 */
export function filterFutureTrips(trips, today = new Date().toISOString().slice(0, 10)) {
  return trips
 ...
 * time advances.
 */
export function filterFutureSegments(segments, today = new Date().toISOString().slice(0, 10)) {
...
export function deduplicateSegments(segments) {
  ...
  // Compare on ISO datetime when present (lexicographic compare works
  // for `...Z`); fall back to date-only for legacy callers that
  // construct segments without datetime fields.
  const startOf = s => s.startDateTime ?? s.startDate;
  const endOf = s => s.endDateTime ?? s.endDate;

  const sorted = [...segments].sort((a, b) => startOf(a).localeCompare(startOf(b)));
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    // Merge same-tz segments that touch or overlap on the calendar.
    // Date-adjacency is intentional here: real-world hotel transitions
    // can leave a small intra-day gap (check-out 11am → check-in 1pm)
    // inside one timezone; that gap is not a TZ change. The merged
    // endDateTime must still track the latest ISO datetime — same
    // endDate can hide a later endDateTime advance that a sub-day-aware
    // consumer needs.
    if (prev.timezone === curr.timezone && prev.endDate >= curr.startDate) {
      if (endOf(curr) > endOf(prev)) {
        prev.endDate = curr.endDate;
        prev.endDateTime = curr.endDateTime;
      }
      prev.label += ` + ${curr.label}`;
    } else {
      // Clip on datetime overlap so a same-calendar-date overlap
      // (e.g. morning arrival in TZ-A and afternoon departure into
      // TZ-B on the same day) gets clipped — date-only comparison
      // misses it. Clip both shapes together so a clipped segment
      // never lies about its own boundary.
      if (endOf(prev) > startOf(curr)) {
        prev.endDate = curr.startDate;
        prev.endDateTime = curr.startDateTime;
      }
      result.push({ ...curr });
    }
  }

  // Drop segments clipped to zero or negative duration. Use datetime
  // when present for precision; date-only fallback for legacy inputs.
  return result.filter(s => startOf(s) < endOf(s));
}

/**
 * Resolve the traveler's home (default) timezone as an IANA string.
 *
 * Reclaim already falls back to the account's home timezone whenever no
 * travel override covers a date, so an override that just restates the
 * home timezone is pure noise. To filter those out we need to know what
 * "home" is.
 *
 * Precedence:
 *   1. `envTz` — explicit `HOME_TZ` env var, authoritative and works in
 *      every mode (including dry-run, which never reaches Reclaim).
 *   2. `reclaimDefault` — the account default reported by Reclaim. It is
 *      usually an IANA string, but the API has been seen to omit it or
 *      wrap it in an object, so we accept a string or pull a string out
 *      of the common object fields. Anything else returns null.
 *
 * Returns a trimmed IANA timezone string, or null when none could be
 * determined (caller should then skip home-timezone filtering rather
 * than guess).
 */
export function resolveHomeTimezone(envTz, reclaimDefault) {
  const env = typeof envTz === 'string' ? envTz.trim() : '';
  if (env) return env;

  if (typeof reclaimDefault === 'string') {
    const tz = reclaimDefault.trim();
    return tz || null;
  }

  if (reclaimDefault && typeof reclaimDefault === 'object') {
    for (const field of ['timezone', 'ianaName', 'id', 'name']) {
      const v = reclaimDefault[field];
...
/**
 * Split segments into those that should be pushed to Reclaim and those
 * that merely restate the home timezone (redundant overrides).
 *
 * When `homeTz` is falsy the home timezone is unknown, so nothing is
 * dropped — filtering on a guess would be worse than a few redundant
 * overrides.
 *
 * Returns { kept, dropped }.
 */
export function filterHomeTimezoneSegments(segments, homeTz) {
  if (!homeTz) return { kept: segments, dropped: [] };

  const kept = [];
  const dropped = [];
  for (const s of segments) {
    (s.timezone === homeTz ? dropped : kept).push(s);
  }
  return { kept, dropped };
}
