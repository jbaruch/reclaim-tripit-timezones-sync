...
describe('proxy routing under OneCLI mode (HTTPS CONNECT)', () => {
  let snap;
  let previousDispatcher;
  let installedAgent;
  let originServer;
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      throw new Error(
        missing
          ? 'HTTPS CONNECT proxy tests require the `openssl` CLI on PATH ' +
            '(used to mint an ephemeral self-signed cert). Install openssl ' +
            'or skip this suite in environments without it.'
          : `openssl failed to generate test cert: ${detail}`,
      );
    }
