...
describe('proxy routing under OneCLI mode (HTTP forward)', () => {
  ...
  it('routes Reclaim-style global fetch through the proxy with a placeholder Bearer', async () => {
    // listEntries hardcodes api.app.reclaim.ai, so we cannot point it at the
    // mock origin without a base-URL seam. createClient + global fetch still
    // exercise the real client headers and undici ProxyAgent path that
    // listEntries uses (same global fetch + Authorization header).
