...
describe('buildTimezoneSegments', () => {
  ...
  it('west-of-UTC last-day late-night flight after tripEnd UTC is still included', () => {
    // Trip ends 06-08 (iCal exclusive midnight UTC). For a Chicago user,
    // last local day extends from 06-07 05:00 UTC through 06-08 05:00 UTC.
    // A return flight at 06-08 03:00 UTC is on the user's last local
    // evening (22:00 CDT 06-07) but past tripEnd UTC. The 12h upper
    // buffer keeps it; a strict bound would drop it.
    const trips = [{ summary: 'Chicago Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-08'), location: 'Chicago', geo: null }];
    const flights = [
      { arrivalDate: new Date('2026-06-01T14:00:00Z'), tzAbbr: 'CDT', arrivalCity: 'Chicago' },
      { arrivalDate: new Date('2026-06-08T03:00:00Z'), tzAbbr: 'CDT', arrivalCity: 'Chicago' },
    ];
    const segments = buildTimezoneSegments(trips, flights);
  ...
  it('east-of-UTC first-day flight before tripStart UTC is still included', () => {
    // Trip starts 06-01 (iCal midnight UTC). For a Tokyo trip, the first
    // local calendar day starts at 05-31T15:00 UTC. A flight arriving at
    // 05-31T22:00 UTC is on the user's first local day but before
    // tripStart UTC. Strict `>= tripStart` would drop it; the lower
    // buffer keeps it.
    const trips = [{ summary: 'Tokyo Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-05'), location: 'Tokyo', geo: null }];
    const flights = [
      { arrivalDate: new Date('2026-05-31T22:00:00Z'), tzAbbr: 'JST', arrivalCity: 'Tokyo' },
    ];
    const segments = buildTimezoneSegments(trips, flights);
