...
describe('buildTimezoneSegments', () => {
  ...
  it('unmatched-checkout first stay: pre-lodging anchored on next check-in', () => {
    ...
    const flights = [
      // Flight arriving during the unmatched-checkout stay — must NOT be pre-lodging.
    ...
    const segments = buildTimezoneSegments(trips, flights, stays);
    // The NY flight should NOT appear as a pre-lodging segment that ends at
    // the orphan checkout — pre-lodging must anchor on the next real check-in.
    const nySeg = segments.find(s => s.timezone === 'America/New_York');
    if (nySeg) {
      // If a NY segment exists at all, it must be in a between-stay or pre-cutoff
      // window strictly bounded by the Real Hotel check-in (06-05), never extending
      // past the orphan's checkout date as a pre-lodging cap.
      assert.ok(nySeg.endDate <= '2026-06-05', 'NY segment must not be capped by orphan checkout');
    }
  });
