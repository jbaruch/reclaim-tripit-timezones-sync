...
it('carry-over unmatched checkout from previous trip does not bleed', () => {
  ...
  it('GEO covers single-sided TZ-less stay (only checkin or only checkout)', () => {
    // TZ-less stay with only a checkin (open-ended). Earlier code required
    // both checkins AND checkouts to fire the partial GEO; with only one
    // side present, the unresolved window stayed uncovered.
    const trips = [{ summary: 'Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-10'), location: 'Cancun', geo: { lat: 21.1619, lon: -86.8515 }, tripId: 'X' }];
    const stays = [
