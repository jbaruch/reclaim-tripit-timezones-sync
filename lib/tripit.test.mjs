...
describe('buildTimezoneSegments', () => {
  ...
  it('between-stay gap-fill skipped when next stay has no check-in', () => {
    // If the next stay only has a checkout (orphan), there's no real
    // start boundary; treating its checkout as the gap end would
    // manufacture flight segments across days the user was at the next
    // hotel.
    const trips = [{ summary: 'Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-15'), location: '', geo: null }];
    const flights = [
      { arrivalDate: new Date('2026-06-07T10:00:00Z'), tzAbbr: 'EDT', arrivalCity: 'New York' },
    ];
    const stays = [
      { hotelName: 'First', checkinDate: new Date('2026-06-02T15:00:00Z'), checkoutDate: new Date('2026-06-05T11:00:00Z'), timezone: 'America/Chicago' },
      // Orphan checkout — no checkin date — treating its checkout as
      // gap-end would create a NY segment over days the user was at this hotel.
      { hotelName: 'Orphan', checkinDate: null, checkoutDate: new Date('2026-06-12T11:00:00Z'), timezone: 'America/Denver' },
    ];
    const segments = buildTimezoneSegments(trips, flights, stays);
    const nySeg = segments.find(s => s.timezone === 'America/New_York');
    assert.equal(nySeg, undefined, 'no between-stay segment should be emitted when the next stay has no checkinDate');
  });
