...
it('carry-over unmatched checkout from previous trip does not bleed', () => {
  ...
  it('post-lodging fires when last stay has no checkout but earlier stay does', () => {
    ...
    it('orphan with no earlier resolved stay does not eclipse pre-lodging flight', () => {
    // Pre-lodging flight covers trip start → first lodging signal. The
    // only stay is an orphan checkout — without seeding the orphan's
    // rawStart from the pre-lodging flight's end, the orphan claims back
    // to rangeStart (trip.startDate) and eclipses the flight after dedup.
    const trips = [{ summary: 'Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-15'), location: '', geo: null, tripId: 'X' }];
    const flights = [
      { arrivalDate: new Date('2026-06-01T14:00:00Z'), tzAbbr: 'EDT', arrivalCity: 'Atlanta', tripId: 'X' },
    ];
    ...
    const dedup = deduplicateSegments(filterFutureSegments(segments));
    const atl = dedup.find(s => s.timezone === 'America/New_York');
    assert.ok(atl, 'pre-lodging Atlanta segment must survive — orphan must not claim back to trip start');
    assert.ok(atl.startDate <= '2026-06-01');
  });
