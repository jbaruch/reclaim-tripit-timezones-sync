...
it('carry-over unmatched checkout from previous trip does not bleed', () => {
  ...
  it('post-lodging fires when last stay has no checkout but earlier stay does', () => {
    // Last stay (in sort order) is open-ended (no checkout). Post-lodging
    // anchors on the LATEST KNOWN checkout across all stays, not just
    // the chronologically last stay's missing checkout.
    const trips = [{ summary: 'Trip', startDate: new Date('2026-06-01'), endDate: new Date('2026-06-15'), location: '', geo: null, tripId: 'X' }];
    const flights = [
      // Return-home flight after the earlier stay's checkout.
    ...
    const stays = [
      ...
      // Sorted later because checkin > First.checkin, but no checkout.
      { hotelName: 'Open-ended Last', checkinDate: new Date('2026-06-06T15:00:00Z'), checkoutDate: null, timezone: 'Europe/London', tripId: 'X' },
    ];
    const segments = buildTimezoneSegments(trips, flights, stays);
    const home = segments.find(s => s.timezone === 'America/Chicago');
    assert.ok(home, 'post-lodging must fire on the latest known checkout, not stop because last stay lacks one');
  });
