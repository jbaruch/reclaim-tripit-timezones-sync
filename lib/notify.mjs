...
export function findOverlaps(segments) {
  ...
  //
  // Dedupe by (labelA, labelB) — within-trip raw overlaps that the
  // lodging-primary algorithm intentionally produces would otherwise
  // surface the same cross-trip pair multiple times, once for each
  // overlapping raw segment of the earlier trip.
  ...
  for (const curr of sorted) {
    for (let k = active.length - 1; k >= 0; k--) {
      if (active[k].endDate <= curr.startDate) active.splice(k, 1);
    }
    for (const a of active) {
      ...
      // Canonicalize the pair key (sorted) so A→B and B→A in different
      // raw-segment orderings dedupe to the same warning.
      const pairKey = nameA < nameCurr ? `${nameA}|${nameCurr}` : `${nameCurr}|${nameA}`;
