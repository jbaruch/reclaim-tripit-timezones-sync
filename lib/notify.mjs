...
export function findOverlaps(segments) {
  ...
  for (const curr of sorted) {
    ...
    for (const a of active) {
      ...
      // Suppress only when BOTH labels are well-formed `Trip - Detail`
      // strings — that's the only signal we can use to attribute segments
      // to a trip. Two unlabeled segments (or label-without-separator
      // segments) might be from different unknown trips; surface the
      // overlap rather than silencing it. Also lets a trip whose summary
      // genuinely is "Other" still get suppression since both sides will
      // carry the same well-formed `Other - X` label.
      const nameA = extractTripName(a.label);
      const nameCurr = extractTripName(curr.label);
      const aIsTripLabeled = !!a.label && a.label.includes(' - ');
      const currIsTripLabeled = !!curr.label && curr.label.includes(' - ');
      if (nameA === nameCurr && aIsTripLabeled && currIsTripLabeled) continue;
