...
describe('findOverlaps', () => {
  ...
  it('suppresses same-trip self-overlap for a trip genuinely named "Other"', () => {
    // A trip whose actual summary is "Other" produces well-formed
    // labels like "Other - X" / "Other - Y". Same-trip suppression
    // must apply here — checking just the extracted name against the
    // string "Other" would falsely keep it (collision with the
    // unlabeled-segment fallback sentinel).
    const segments = [
      { startDate: '2026-06-01', endDate: '2026-06-10', timezone: 'America/Chicago', label: 'Other - Lodging' },
      { startDate: '2026-06-02', endDate: '2026-06-08', timezone: 'Europe/London', label: 'Other - GapFill' },
    ];
    const overlaps = findOverlaps(segments);
    assert.equal(overlaps.length, 0, 'a trip named "Other" must still self-suppress');
  });
