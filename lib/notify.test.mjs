...
describe('findOverlaps', () => {
  ...
  it('canonicalizes trip-pair key (A→B and B→A dedupe to same warning)', () => {
    // Two cross-trip overlaps in opposite orderings — without sorted key,
    // the dedup set would store both `A|B` and `B|A` and emit twice.
    const segments = [
      ...
      // A second pair where Trip B starts before Trip A — would produce
      // a `B|A` key without canonicalization, in addition to the `A|B`.
    ...
    assert.equal(aVsB.length, 1, 'A↔B pair must dedupe regardless of which side starts first');
  });
