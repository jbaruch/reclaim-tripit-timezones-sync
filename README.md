...
The plugin is versioned (see `CHANGELOG.md`). The version lives in
`.tessl-plugin/plugin.json`. The agent install URL in
`skills/sync-tripit/SKILL.md` points at a tagged tarball, so installs pull
the exact version the plugin expects rather than whatever happens to be on
`main`.

### Automated flow

`.github/workflows/publish-plugin.yml` runs on every push to `main` that
touches the plugin content (`.tessl-plugin/**`, `rules/**`, `skills/**`,
`evals/**`, `README.md`). It uses [`tesslio/setup-tessl`][gha] to install
the Tessl CLI, then runs `tessl plugin lint` and `tessl plugin publish`.

The published version is always exactly what's in `.tessl-plugin/plugin.json`
— unlike tiles, plugin publish does not auto-bump the patch number. So the
version is whatever you set in the manifest.

When a push changes the `plugin.json` version, the workflow creates and
pushes the matching `vX.Y.Z` git tag BEFORE publishing, so the in-skill
`curl ... archive/refs/tags/vX.Y.Z.tar.gz` URL resolves the moment the
registry has the new version. Pushes that don't change the version are
no-ops on the tagging side.

[gha]: https://github.com/tesslio/setup-tessl
...
Every release is explicit — set the version, then push:

```bash
# 1. Bump version + propagate the install URL to SKILL.md, eval criteria, research.md
scripts/bump-version.sh 0.3.0

# 2. Add an entry to CHANGELOG.md, commit, open a PR, merge
```

The publish-plugin workflow takes over from there: it pushes the
`v0.3.0` tag at the merge commit and publishes the plugin to the
registry. No manual tagging needed.
...
- The **Tessl registry** is what `tessl install jbaruch/reclaim-tripit-sync`
  resolves to. It carries the plugin bundle (rules + skills + manifest).

Already-installed agent plugins continue to work on whatever version they
